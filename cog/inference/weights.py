...
class WeightsDownloadCache:
    def __init__(self, min_disk_free: int = 5 * (2**30), base_dir: str = "/src/weights"):
        """
    ...
    def weights_path(self, url: str) -> str:
        ...
        return os.path.join(self.base_dir, f'{short_hash}.safetensors')
    ...
    def download_weights(self, url: str, dest: str) -> None:
        ...
        st = time.time()
        try:
            output = subprocess.check_output(["wget", "-q", url, '-O', dest])
            print(output)
