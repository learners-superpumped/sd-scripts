...
class WeightsDownloadCache:
    def __init__(
        self,
        min_disk_free: int = 5 * (2**30),
        base_dir: str = "/src/weights",
        ...
        # Add file path that already existed.
        files = glob.glob(os.path.join(base_dir, "*"))
        self.lru_paths.appendleft(files)
    ...
    def weights_path(self, url: str) -> str:
        ...
        return os.path.join(self.base_dir, f'{short_hash}.{url.split(".")[-1]}')
    ...
    def download_weights(self, url: str, dest: str) -> None:
        ...
        st = time.time()
        try:
            if "stable-diffusion-server" in url:
                chunks = url.split("/")
                bucket_index = [
                    i
                    for i in range(len(chunks))
                    if "stable-diffusion-server" in chunks[i]
                ][0]
                bucket = self.storage_client.bucket(chunks[bucket_index])
                blob = bucket.blob("/".join(chunks[bucket_index + 1 :]))
                blob.download_to_filename(dest)
            else:
                output = subprocess.check_output(["wget", url, "-O", dest])
                print(output)
