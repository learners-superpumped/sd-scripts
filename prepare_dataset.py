...
def crop_face(image: Image.Image, crop_height_rate: float = 1.0, crop_width_rate: float = 1.0):
...
def main(args):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        for crop_rate in [1.0, 1.2, 1.5, 2.0]:
            try:
                crop_left, crop_top, crop_right, crop_bottom, face = crop_face(image, crop_height_rate=crop_rate, crop_width_rate=crop_rate)
            except ValueError:
                continue
            face = face.resize((args.save_size, args.save_size))
            filename =  f"cr-{crop_rate}-{os.path.basename(image_path)}"
            print(f"Saving {filename}...")
            face.save(os.path.join(args.output_dir, filename))
